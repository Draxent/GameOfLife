...
void copyBorder()
{	
	// Fill the bottom bsorder
	MREAD[numCells - 1] = MREAD[side + 1];
	copy( MREAD + side + 1, MREAD + 2*side - 1, MREAD + numCells - side + 1 );
	MREAD[numCells - side] = MREAD[2*side - 2];
	
	// Fill the top border
	MREAD[side - 1] = MREAD[numCells - 2*side + 1];
	copy( MREAD + numCells - 2*side + 1, MREAD + numCells - side - 1, MREAD + 1 );
...
void print_matrix( string name, bool* M, bool border = false )
{
	int add = border ? 0 : 1;
	cout << name << ":" << endl;
...
int main( int argc, char* argv[] )
{
	...
	try
	{
		MREAD = new bool[numCells];
		MWRITE = new bool[numCells];
	}
